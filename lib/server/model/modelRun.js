...
class Model extends ModelBase {
  ...
  async preSave(iu, database, pgschema, user) {
    ...
    for (let i=0,col,fldType,sviu; i<colNames.length; i++) {
      col = colNames[i];
      fldType = cols[col].constructor.name;

      if (iu == 'i') {
        // insert
        sviu = cols[col].defn.onBeforeInsert || cols[col].defn.onBeforeUpsert || null
      }
      else {
        // update
        sviu = cols[col].defn.onBeforeUpdate || cols[col].defn.onBeforeUpsert || null;
      }
...
/* Instance actions */  
  ...
  async insertOne({database = '', pgschema = '', user = {}} = {}, trans) {
    // insert model object
    ...
    tm = (trans) ? await trans.exec({text, values}) : await db.exec(database, {text, values});
  ...
  async updateOne({database = '', pgschema = '', user = {}} = {}, trans) {
  ...
  async upsertOne({database = '', pgschema = '', user = {}} = {}, trans) {
  ...
  async deleteOne({database = '', pgschema = '', user = {}} = {}, trans) {
    ...
    if (rec == false) return userError('Missing PK field ' + SQLTableName + ' deleteOne');
...
/* Model actions */  
  static async selectOne({database = '', pgschema = '', user = {}, cols = '*', showHidden = false, pks = []} = {}, trans) {
    ...
    if (Object.values(rec).indexOf(null) > -1) return userError('Missing PK field ' + SQLTableName + ' selectOne');
    ...
    if (tm.status == 200) {
      if (tm.data.length != 1) {
        tm.status = 400;
        tm.message = `More/less than one row returned from ${SQLTableName}`;
      }
      else {
        tm.data = tm.data[0];
      }
  ...
  static async select({database = '', pgschema = '', user = {}, rec = {}, cols = '*', limit=100, offset=0, orderby='', showHidden = false} = {}, trans) {
    ...
    return tcon.where({database, pgschema, where, values, cols, limit, offset, orderby, showHidden}, trans);
  }
  
  static async where({database = '', pgschema = '', user = {}, where = '', values = [], cols = '*', limit=100, offset=0, orderby='', showHidden = false} = {}, trans) {
    ...
    tm = (trans) ? await trans.exec({text, values}) : await db.exec(database, {text, values});
  //console.log(text, values, JSON.stringify(trans).substring(1,200))
    return tm;
  }

  static async insert({database = '', pgschema='', user = {}, rows=[]} = {}, trans) {
    // insert objects
  ...
  static async update({database = '', pgschema, user = {}, rows=[]} = {}, trans) {
    // update objects
  ...
  static async delete({database = '', pgschema = '', user = {}, obj = {}} = {}, trans) {
