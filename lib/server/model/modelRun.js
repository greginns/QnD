...
const {DataValidationError, RowNotInsertedError, RowNotUpdatedError, SQLSelectError, SQLDeleteError, SQLSchemaError, UserError} = require(root + '/lib/server/utils/errors.js');
...
/* Instance actions */  
  ...
  async updateOne({pgschema = ''} = {}) {
    ...
    if (!tm.err) {
      if (tm.data.length == 0) {
        tm.err = new RowNotUpdatedError(text);
      }
      else {
