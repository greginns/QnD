...
const {DataValidationError, SQLInsertError, SQLUpdateError, SQLSelectError, SQLDeleteError, SQLSchemaError, UserError} = require(root + '/lib/server/utils/errors.js');
...
/* Instance actions */  
  ...
  async updateOne({pgschema = ''} = {}) {
    ...
    text = `UPDATE ${table} SET ${set.join(',')} WHERE ${where.join(' AND ')} RETURNING ${retcols};`;
    tm = await tcon.sql({text, values});    
  ...
  async deleteOne({pgschema = ''} = {}) {
    ...
    text = `DELETE FROM ${table} WHERE ${where.join(' AND ')} RETURNING ${retcols};`;
    tm = await tcon.sql({text, values});  
...
/* Model actions */  
  static async selectOne({pgschema = '', cols = '*', showHidden = false, pks = []} = {}) {
    ...
    text = `SELECT ${colNames} FROM ${table} WHERE ${where.join(' AND ')}`;
    tm = await tcon.sql({text, values});
  ...
  static async select({pgschema = '', rec = {}, cols = '*', query = {}, showHidden = false} = {}) {
    ...
    colNames = tcon.getColumnList({cols, isMainTable: true, showHidden});
    table = tcon.getTableName({pgschema});
    orderBy = tcon.makeOrderBy(query.orderby);  // comma separated list with direction, ie -last,first
    
    let [where, values] = tcon.makeSelectValues(rec);
    text = `SELECT ${colNames} FROM ${table}` + ((where.length > 0) ? ` WHERE ${where.join(' AND ')}` : '');
    text += ' ' + orderBy;
    
    if ('limit' in query && query.limit) text += ' LIMIT ' + query.limit;
    if ('offset' in query && query.offset) text += ' OFFSET ' + query.offset;
