...
class Model extends ModelBase {
  ...
  async preSave(iu) {
    ...
    for (var i=0,col,fldType,sviu; i<colNames.length; i++) {
      ...
      // Date-ize date/time fields
      /*
      if (['Date', 'Time'].indexOf(fldType) > -1) {
        if (this[col]) {
          let dt = this[col].split('T');
          this[col] = (fldType == 'Time') ? dt[1] : dt[0]
        }
      }
        */    
      // Set any onBeforeInsert/Update/Upsert values
      if (sviu) {
        this[col] = (typeof sviu === 'function') ? sviu(this[col]) : sviu;
      }
    }
  }    
