  ...
  async updateOne({pgschema = ''} = {}) {
    ...
    if (tm.status == 200) {
      if (tm.data.length == 0) {
        ...
        modelEvents.emit(`${tenant}./${appName}/${tableName}`, {action: '*', rows: tm.data});
console.log(`${tenant}./${appName}/${tableName}`)
        tm.data = tm.data[0];
      }
    }
...
/* Model actions */  
  ...
  static async select({pgschema = '', rec = {}, cols = '*', limit=100, offset=0, orderby='', showHidden = false} = {}) {
    ...
    return tcon.where({pgschema, where, values, cols, limit, offset, orderby, showHidden});
  }
  
  static async where({pgschema = '', where = '', values = [], cols = '*', limit=100, offset=0, orderby='', showHidden = false} = {}) {
