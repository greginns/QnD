...
*/
  ...
  static getColumnDefaults() {
    ...
    for (let col in schema) {
      defaults[col] = ('default' in schema[col].defn) ? typeof schema[col].defn.default === 'function' ? schema[col].defn.default() : schema[col].defn.default : '';
    }
  ...
  static getColumnListBasic(cols) {
    ...
    if (!cols || cols == '*' || Array.isArray(cols) && cols[0] == '*') {
      cols = this.getColumnNames({showHidden: false, includeDerived: true});
    }
    else {
      if (!Array.isArray(cols)) cols = [cols];

      // filter list in case of bad names
      colNames = this.getColumnNames({showHidden: true, includeDerived: true}); // All columns - as reference.
