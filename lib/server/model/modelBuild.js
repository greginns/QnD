    ...
    var buildIndexes = function() {
      ...
      if ('index' in constraints && constraints.index.length > 0) {
        constraints.index.forEach(function(val) {
          ...
          let unique = (val.unique) ? ' UNIQUE' : '';
              
          indexes.push(` CREATE${unique} INDEX "${indexName}" ON ${tableName} (${val.columns.join(',')})`);
        })  
      }
      ...
      var uniqueAdd = function() {
        ...
        (constraints.unique || []).forEach(function(nval) {
          ...
          if (addUnique) {
            ...
            indexes.push(` CREATE UNIQUE INDEX "${uniqueName}" ON ${tableName} (${jnvals.columns.join(',')})`);
          }
        })
      };
