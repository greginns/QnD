...
class ModelBuild {
  ...
  create(pgschema) {
    ...
    var buildIndexes = function() {
      ...
      if ('index' in constraints && constraints.index.length > 0) {
        constraints.index.forEach(function(val) {
          indexName = self.model.makeIndexName(`${pgschema}.${tableNameRaw.toLowerCase()}`, val.name);
    ...
    if (fks.length > 0 || indexes.length > 0) {
    ...
    var alterConstraints = function() {
      ...
      var indexAdd = function() {
        ...
        (constraints.index || []).forEach(function(nval) {
          ...
          if (addIndex) {
            let indexName = self.model.makeIndexName(tableName, nval.name);
      ...
      var indexDrop = function() {
        ...
        if ('constraints' in old && 'index' in constraints) {
          (old.constraints.index || []).forEach(function(oval) {
            ...
            if (dropIndex) {
              indexName = self.model.makeIndexName(tableName, oval.name);
            ...
            valArray.forEach(function(val) {
              if (typeof val != 'object' || !val) {
                ...
                if (indexNames.indexOf(val.name.toLowerCase()) > -1) {
                  errs.push(`${tableName} : Index ${val.name} must be unique`);
                }

                if ('columns' in val && !val.columns) {
                  errs.push(`${tableName} : Index Columns must have a value`);
                }
                else {
                  if (!Array.isArray(val.columns)) {
                    errs.push(`${tableName} : Index Columns must be an array of column name(s)`);
                  }
                  else {
                    for (let col of val.columns) {
                      if (colNames.indexOf(col) == -1) {
                        errs.push(`${tableName} : Index Column ${col} must be a valid column name`);
                      }
                    }
                  }
                }
