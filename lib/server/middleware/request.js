...
rewrite.set('/favicon.ico', '/static/v0/favicon.ico');
...
//console.log('END', aborted)      
      ...
      if (ct == 'application/json') {
        try {
          ...
          var rm = new ResponseMessage({err: new JSONError(err), status: 500});    
          reject(rm);
        }
      }
...
function rewritePath(path) {
  ...
  const parts = path.split('/');  // always ["", ...];
  
  if (rewrite.has(path)) return rewrite.get(path);
  if (parts.length == 1) return `/${config.defaultApp}/v0`;  // [''], ie /
  if (config.apps.indexOf(parts[1]) == -1) parts.splice(1,0,config.defaultApp);  // no app.
  if (!versionRe.test(parts[2])) parts.splice(2,0,'v0');  // no version
