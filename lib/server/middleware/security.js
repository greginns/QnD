  ...
  check: async function(req, res) {
    ...
    const testRoles = function(userGroup) {
      ...
      if (id) {
        ...
        if (idp.length > 2) {
          return Authorization.canUserAccess(idp[0], idp[1], idp[2], userGroup);
        }
      }
    ...
    if (rm.status == 200) {
      if (!testRoles('admin')) {  // *** user.group, ie rm.data.user.group
        // no access
        rm = new ResponseMessage({status: 401, err: 'Invalid Role'});
      }      
    }
