...
const migration = async function({database = '', pgschema = 'public', migApp = null} = {}) {
  // run migration for one pgschema
  var model, sql, sqlFK, fks = [], res, tableName, tm;
  var jsonModels, errs = [], verrs = [];
  var modelsList = {}, appNames = [];
  let filename = './' + (new Date()).toJSON() + '.sql';
  filename = filename.replace(new RegExp(':', 'g'), '-');

  tm = new TravelMessage();

  if (pgschema == 'public' && !migApp) migApp = 'admin';

  if (pgschema == 'public' && migApp && migApp != 'admin' && migApp != 'schema' && migApp != 'db4admin') {
  ...
  for (let app of appNames) {
    for (let model of Object.keys(modelsList[app])) {
      if (!modelsList[app][model].name) continue;
  ...
  for (let app of appNames) {
    ...
    let migrationFile = folder + `/${pgschema}.json`;
    ...
    for (let model of Object.keys(modelsList[app])) {
      ...
      if (! (model in jsonModels)) {
        var [verrs, sql, sqlFK] = mb.create(pgschema);
      }
      else {
        //var [verrs, sql] =  mb.alter(pgschema, jsonModels[model]);
        sqlFK = '';
        sql = '';
      }
      ...
      if (sql) {
        ...
        res = await sqlUtil.execQuery(database, sql);
    ...
    // now create all FKs
    for (var fk of fks) {
      fs.appendFile(filename, fk + '\n');

      res = await sqlUtil.execQuery(database, fk);
...
/*  
  ...
  for (let app of appNames) {
    let migrationFile = root + `/apps/${app}/migrations/${pgschema}.json`;
    ...
    for (model of jsonNames) {
      if (modelNames.indexOf(model) == -1) {
        ...
        tableName = `"${pgschema}"."${app}_${model}"`
