...
class SqlBuilder {
  constructor(pgschema, user, pswd) {
  ...
  createUser() {
    if (!this.user) return this._missing('User');
    if (!this.pswd) return this._missing('Password');

    let sql = `CREATE USER "${this.user}" PASSWORD "${this.pswd}"`;
    sql += `ALTER USER "${this.user}" SET search_path = "${this.pgschema}";`;
    sql += `REVOKE ALL PRIVILEGES ON SCHEMA public FROM "${this.user}"`
  ...
  createSchema() {
    ...
    let sql = `CREATE SCHEMA IF NOT EXISTS "${this.pgschema}";`;
    sql += `GRANT USAGE ON SCHEMA "${this.pgschema}" TO "${this.user}";`;
