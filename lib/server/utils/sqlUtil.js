...
const jsonQueryExecify = async function({query={}, app='', pgschema = '', values=[]} = {}) {
...
const execQuery = async function(stmt) {
  return await db.exec(stmt);
};
...
class SqlBuilder {
  ...
  createSchema() {
    ...
    return [`CREATE SCHEMA IF NOT EXISTS "${this.pgschema}";`];
  }

  dropSchema() {
    if (!this.pgschema) return this._missing('Schema');

    return [`DROP SCHEMA IF EXISTS "${this.pgschema}" CASCADE;`];
  }
  ...
  createPK(appName, tableName, pkParams) {
    ...
    let fullName = this._makeFullTableName(appName, tableName+'_pkey');
    let alter = this._makeAlterTable(appName, tableName);
  ...
  createFK(appName, tableName, fkParams, dropOnly) {
    if (!this.pgschema) return this._missing('Schema');
    if (!appName) return this._missing('App Name');
    if (!tableName) return this._missing('Table Name');
    ...
    let ffullName = this._makeFullTableName(fkParams.app, fkParams.ftable);

    let alter = this._makeAlterTable(appName, tableName);
    let columns = [], fcolumns = [];

    for (let link in fkParams.links) {
  ...
  dropFK(appName, tableName, fkParams) {
    return this.createFK(appName, tableName, fkParams, true);
  }

  createIndex(appName, tableName, idxParams, dropOnly) {
    // indexes are DB wide, so need to namespace with schema and table names
    // *** Perhaps make sure name is unique for schema then index can be schema+name, 36+x<=63, x=27
    if (!this.pgschema) return this._missing('Schema');
    if (!appName) return this._missing('App Name');
    if (!tableName) return this._missing('Table Name');
    ...
    let fullName = this._makeFullTableName(appName, tableName);
    let conName = this._makeFullTableName(appName, `${tableName}_${idxParams.name}_index`);

    let alter = `DROP INDEX IF EXISTS ${conName}`;    
...
class ColumnBuilder {
  ...
  static date(colParams) {
    ...
    if ('defaultDD' in colParams && colParams.defaultDD) {
      if (defaultDD == 'U') sql+= ` DEFAULT '${colParams.defaultDD2}'`;
    } 
