...
const jsonToQuery = function(query, dfltApp, pgschema) {
  ...
  */
  ...
  let makeMainColumns = function(tbl, app, cols, derived) {
    ...
    for (let col of derived) {
      let defn = models[app][tbl].getColumnDefn(col);

      colList.push(`${defn.defn} AS ${col}`);
    }
  }

  let makeJoinColumns = function(tbl, app, cols, derived, pfx, as) {
    let tname = models[app][tbl].getTableNamePhysical(true);
    let basic = (as) ? as : tbl;
    ...
    for (let col of derived) {
      ...
      colList.push(`${defn.defn} AS "${basic}.${col}"`);
    }
  }
  ...
  let swallow = function(tblA, tblADefn, appA, isMainTable, pfx) {
    let reqCols = ('columns' in tblADefn) ? tblADefn.columns : ['*'];
    let as = tblADefn.as || null;

    let cols = models[appA][tblA].getColumnListBasic(reqCols);
    let derived = models[appA][tblA].getColumnListDerived(reqCols);

    (isMainTable) ? makeMainColumns(tblA, appA, cols, derived) : makeJoinColumns(tblA, appA, cols, derived, pfx, as);    
  ...
  flat.forEach(function(row) {
    ...
    Object.keys(row).forEach(function(key) {
      ...
      for (let k of keyParts) {
        if (! (k in mdl) || !utils.object.isObject(mdl[k])) mdl[k] = {};
...
class SqlBuilder {
  ...
  createPK(appName, tableName, pkParams) {
    ...
    let fullName = `"${appName}_${tableName}_pkey"`;
    let alter = this._makeAlterTable(appName, tableName);
