...
const jsonToQuery = function(query, app, pgschema) {
  ...
  let doJoin = function(tblA, tblB, jon, onName) {
    if (!jon) {
      var jona = [];
      var fks = models[app][tblA].getConstraints().fk;
      
      jon = `LEFT JOIN "${pgschema}"."${app}_${tblB}" AS "${app}_${tblB}" ON `;
      
      fks.forEach(function(fk) {
        if (fk.table.name == tblB && (!onName || onName == fk.name)) {  // if specified on Name use that, else do all with same table name
          let colsA = fk.columns;
          let colsB = fk.tableColumns;   
          
          colsA.forEach(function(col, idx) {
            jona.push(`"${app}_${tblA}"."${col}" = "${app}_${tblB}"."${colsB[idx]}"`);
          })
  ...
  let swallow = function(tblA, tblADefn, isMainTable, joinName) {
    ...
    var cols = models[app][tblA].getColumnList({cols: tblADefn.columns, isMainTable, joinName, query:true})

    if (cols.length > 0) colList = colList.concat(cols);  
    
    (tblADefn.leftJoin || []).forEach(function(ij) {
      let tblB = Object.keys(ij)[0];
      let onName = ij[tblB].name || '';
      let jon = ij[tblB].on || '';
      
      joins.push(doJoin(tblA, tblB, jon, onName));
      
      swallow(tblB, ij[tblB], false, true);
    })   
  }

  swallow(mainTable, query[mainTable], false, false);    
