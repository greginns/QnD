...
class Routes {
  ...
  constructor({app='', subapp='', version='v1', model={}, services={}, allowCORS=true, pgschema=null} = {}) {
    this.app = app;
    this.subapp = subapp;
    this.version = version;
    this.model = model;
  ...
  getMany() {
    Router.add(new RouterMessage({
      ...
      fn: async function(req) {
        ...
        let tm = await this.services[this.subapp].getMany({pgschema: this.pgschema || req.TID, rec, cols, where, values, limit, offset, orderby});
    ...
    Router.add(new RouterMessage({
      ...
      fn: async function(req) {
        ...
        if (!id) {
          ...
          tm = await this.services[this.subapp].getOne({pgschema: this.pgschema || req.TID, rec: {id} });
  ...
  create() {
    Router.add(new RouterMessage({
      ...
      allowCORS: this.allowCORS,
      fn: async function(req) {
        let tm = await this.services[this.subapp].create({pgschema: this.pgschema || req.TID, rec: req.body[this.subapp] || {} });
    ...
    Router.add(new RouterMessage({
      ...
      fn: async function(req) {
        ...
        if (!id) {
          ...
          tm = await this.services[this.subapp].update({pgschema: this.pgschema || req.TID, id, rec: req.body[this.subapp] || {} });
    ...
    Router.add(new RouterMessage({
      ...
      fn: async function(req) {
        ...
        if (!id) {
          ...
          tm = await this.services[this.subapp].delete({pgschema: this.pgschema || req.TID, id});
