...
class Router {
  ...
  static add(msg) {
    ...
    // save auths
    if (msg.id) {
      let idp = msg.id.split('.');

      if (idp.length > 2) {
        Authorization.addActionLevel(idp[0], idp[1], idp[2], msg.level || 0);
      }
    }
  }
...
class RouterMessage {
  constructor({method='post', app='', version='v0', path='', id='', level=0, desc='', resp={}, rewrite=false, fn='', security={}} = {}) {
    /*
      security: {
        strategies: [
