...
class Routes {
  ...
  getDBPG(req) {
    let database = this.database || req.session.data.database;
    let pgschema = this.pgschema || req.session.data.pgschema;
  ...
  getMany() {
    Router.add(new RouterMessage({
      ...
      fn: async function(req) {
        let {database, pgschema} = this.getDBPG(req);
        let {rec, cols, where, values, limit, offset, orderby} = urlQueryParse(req.query);
        let tm = await this.services.getMany({database, pgschema, rec, cols, where, values, limit, offset, orderby});
...
//console.log('getOne',this.model, this.makePath(), rec, havePK)    
        if (!havePK) {        // !id
          tm = new TravelMessage({data: {message: 'Invalid ID'}, status: 400});
        }
        else {
          tm = await this.services.getOne({database, pgschema, rec });  // : {id}
  ...
  create() {
    Router.add(new RouterMessage({
      ...
      fn: async function(req) {
        let {database, pgschema} = this.getDBPG(req);
        let tm = await this.services.create({database, pgschema, rec: req.body[this.subapp] || {} });
    ...
    Router.add(new RouterMessage({
      ...
      fn: async function(req) {
        ...
        if (!havePK) {    // id
          ...
          tm = await this.services.update({database, pgschema, pks, rec: req.body[this.subapp] || {} });  // id
    ...
    Router.add(new RouterMessage({
      ...
      fn: async function(req) {
        ...
        if (!havePK) {
          ...
          tm = await this.services.delete({database, pgschema, pks}); //id
