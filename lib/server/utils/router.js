...
class Routes {
  ...
  getMany() {
    Router.add(new RouterMessage({
      ...
      fn: async function(req) {
        ...
        let tm = await this.services[this.subapp].getMany({database, pgschema, rec, cols, where, values, limit, offset, orderby});
    ...
    Router.add(new RouterMessage({
      ...
      fn: async function(req) {
        ...
        if (!id) {
          ...
          tm = await this.services[this.subapp].getOne({database, pgschema, rec: {id} });
  ...
  create() {
    Router.add(new RouterMessage({
      ...
      fn: async function(req) {
        let {database, pgschema} = this.getDBPG(req);
        let tm = await this.services[this.subapp].create({database, pgschema, rec: req.body[this.subapp] || {} });
    ...
    Router.add(new RouterMessage({
      ...
      fn: async function(req) {
        ...
        if (!id) {
          ...
          tm = await this.services[this.subapp].update({database, pgschema, id, rec: req.body[this.subapp] || {} });
    ...
    Router.add(new RouterMessage({
      ...
      fn: async function(req) {
        ...
        if (!id) {
          ...
          tm = await this.services[this.subapp].delete({database, pgschema, id});
