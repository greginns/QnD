...
*/
  ...
  MVC._addFilter('tag', function(el, value) {
    ...
    let fmttag = el.getAttribute('tag-format') || null;
...
*/
  ...
  MVC._addBinding('value', function(el) {
    ...
    let proxyIdx = this._addProxyFunction(path, function value(val) {
      let inst = mobiscroll.getInst(el);
      if (!inst) return;
...
*/
  ...
  MVC._addBinding('checked', function(el) {
    ...
    if (checkbox) {
      ...
      let proxyIdx = this._addProxyFunction(path, function checked(val) {
        ...
        if (inst && inst.checked != val) inst.checked = val;
      });
      ...
      el.addEventListener('change', function(ev) {  // click
        ...
        self.$updateModel(path, inst.checked);
      });
    }
    else {  // radio
      // from Model
      ...
      el.addEventListener('change', function(ev) {  // click
        ...
        if (inst.checked) {
          this.$updateModel(path, inst.value);
        }
      }.bind(this));
    }
  ...
  MVC._addBinding('options', async function(el) {
    let pfx = MVC.prefix;
    let self = this;
    let path = el.getAttribute(pfx + '-options');
    let textVal = el.getAttribute('textField') || 'text';
    let valueVal = el.getAttribute('valueField') || 'value';

    if (!path) return;

    let proxyIdx = this._addProxyFunction(path+'.length', async function options(val) {
      let inst = mobiscroll.getInst(el);
      if (!inst) return;
      let values = self.$readModel(path);

      let data = [];

      for (let v of values) {
        data.push({text: v[textVal], value: v[valueVal]});
      }

      inst.setOptions({data});

      // *** Shouldn't be needed, Mobi bug
      el.options.length = 0;
      data.forEach((item) => {
        el.options.add(new Option(item.text, item.value));
      });      
    });
  ...
  MVC._addBinding('error', async function(el) {
    let pfx = MVC.prefix;
    let self = this;
    let path = el.getAttribute(pfx + '-error');

    if (!path) return;

    let proxyIdx = this._addProxyFunction(path, async function error(val) {
      let inst = mobiscroll.getInst(el);
      if (!inst) return;

      inst.setOptions({error: val != '', errorMessage: val});
    });
  ...
  MVC._addBinding('text', async function(el) {
    ...
    let proxyIdx = this._addProxyFunction(path, function text(val) {
