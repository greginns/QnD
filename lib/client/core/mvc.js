...
class MVC {
  ...
  _processElement(el) {
    ...
    const noBindingList = ['else', 'handle', 'nohandle', 'edit', 'role', 'filter', 'index', 'proxyidx', 'proxypath'];
    let pfx = MVC.prefix;
      ...
      switch (obj.type) {
        ...
        case 'ObjectLiteral':
          ...
          for (let p=0; p<obj.properties.length; p++) {
            let v = await parse(obj.properties[p].value);
            let k = await parse(obj.properties[p].name);
...
class MVCProxyHandler {
  ...
  _setValue(target, key, value) {
    ...
    switch(valueType) {
      case 'array':
        target[key] = this._makeProxy('array', key);
      ...
      case 'object':
        target[key] = this._makeProxy('object', key);
  ...
  _makeProxy(base, key) {
    let obj = (base == 'object') ? {} : [];
    let proxyFor = (this._proxyFor) ? `${this._proxyFor}.${key}` : key;
