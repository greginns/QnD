...
class MVC {
  ...
  $addMethod(methodName, methodFunc) {
...
MVC._addRole('stepperadj', function(el) {
  ...
  const min = parseFloat(stepper.getAttribute('min') || '1');
  const max = parseFloat(stepper.getAttribute('max') || '999');
...
MVC._addBinding('optgroup', function(el) {
  let path = el.getAttribute('mvc-optgroup');
...
MVC._addBinding('each', function(el) {
  ...
  */
  ...
  let path = el.getAttribute('mvc-each');
  let maxArrayLength = 0;

  if (!path) return;

  // build regexp from index variable.
  let indexVar = el.getAttribute('mvc-index') || '$index';
...
var eventBinder = function(event, fn, el) {
  ...
  if (!(fn in this)) {
    console.log('No',desc,fn,this._section)
  }
  else {
...
MVC._addBinding('xif', function(el) {
  let expr = el.getAttribute('mvc-xif');
