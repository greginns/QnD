...
class MVC {
  ...
  $focus(path) {
    ...
    this._section.querySelector(`[mvc-value="${path}"]`).focus();
  }

  $setValue(path, val) {
    // focus on field with mvc-value=path
    setTimeout(function() {
      this._section.querySelector(`[mvc-value="${path}"]`).value = val;
    }.bind(this),1)    
  }
...
MVC._addFilter('date', function(el, value) {
  ...
  return utils.datetime.JSONToStringDate(value, App.dateFormat);
});

MVC._addFilter('time', function(el, value) {
  // value is a JS date.toJSON() format
  return utils.datetime.JSONToStringTime(value, App.timeFormat);
});
...
MVC._addEdit('date', function(el, value) {
  ...
  return utils.datetime.stringDateToJSON(value, oldVal, App.dateFormat);
});
...
MVC._addEdit('time', function(el, value) {
  ...
  return utils.datetime.stringTimeToJSON(value, oldVal, App.dateFormat);
});
...
MVC._addRole('stepperadj', function(el) {
  const stepper = el.closest('div.input-group').querySelector('input');
  const step = parseFloat(stepper.getAttribute('step') || '1');
  const dir = el.getAttribute('dir') || '+';
  const p = String(step).split('.');
  const tens = '10000000000000000';
  const decs = (p.length < 2) ? 1 : p[1].length+1;  // any decimals?
  const path = stepper.getAttribute('mvc-value');
  const mult = tens.substr(0,decs);
...
MVC._addRole('passwordtoggle', function(el) {
  const eye = el.querySelector('span.eye');
  const eyeslash = el.querySelector('span.eye-slash');
  const password = el.closest('div.input-group').querySelector('input');
...
MVC._addBinding('value', function(el) {
  let eventType = el.getAttribute('mvc-event-type') || 'change';
  let path = el.getAttribute('mvc-value');
...
MVC._addBinding('dhtml', function(el) {
  ...
  let path = el.getAttribute('mvc-dhtml');
...
MVC._addInterface('date:boot', async function(ev) {
  ...
  let mdt = utils.datetime.makeMomentDate(dt);
  let ret = await Module.widgets.mdate.calendar(mdt);
...
MVC._addInterface('time:boot', async function(ev) {
  ...
  let dt = this.$readModel(path);
  let mdt = utils.datetime.makeMomentDate(dt);
  let ret = await Module.widgets.mtime.time(mdt);
