...
class MVC {
  ...
  $addWatched(path, fn) {
    ...
    (function() {
      ...
      this._section.addEventListener(`${path}-FM`, function(ev) {
        if (ev.detail !== oldValue) fn(ev.detail, oldValue);
...
MVC._addEdit('date', function(el, value) {
  ...
  if (value) {
    if (haveMoment) {
      dt = window.moment(value, QnD.dateFormat);
...
MVC._addEdit('time', function(el, value) {
  ...
  if (value) {
    if (haveMoment) {
      tm = window.moment(value, QnD.timeFormat);
...
MVC._addEdit('phone', function(el, value) {
  // if an invalid value, reset to saved value, reposition cursor
  ...
  if ('0123456789 '.indexOf(value.substr(value.length-1,1)) == -1) {
    // invalid value
    posn = el.selectionStart;       // cursor posn
    value = oldVal;                 // set back old value
    setTimeout(function() {
...
MVC._addRole('phone', function(el) {
  el.setAttribute('mvc-filter', 'phone');
  el.setAttribute('mvc-edit', 'phone');
});
