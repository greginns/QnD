...
MVC._addFilter('date', function(el, value) {
  ...
  if (value) {
    if (haveMoment) {
      dt = window.moment(value);
      dtx = dt.format(App.dateFormat);
    }
    else {
      dt = new Date(value);
...
MVC._addFilter('time', function(el, value) {
  // expects a JS date.toJSON() format
  var tm, tmx='';
  var haveMoment = window.moment || null;

  if (value) {
    if (haveMoment) {
      tm = window.moment(value);
      tmx = tm.format(App.timeFormat);
    }
    else {
      tm = new Date(value);
...
MVC._addEdit('date', function(el, value) {
  ...
  if (value) {
    if (haveMoment) {
      dt = window.moment(value, App.dateFormat);
...
MVC._addEdit('time', function(el, value) {
  ...
  if (value) {
    if (haveMoment) {
      tm = window.moment(value, App.timeFormat);
...
MVC._addBinding('dhtml', function(el) {
  ...
  this._section.addEventListener(`${path}-FM`, function(ev) {
    ...
    el.appendChild(this._processFilters(el, ev.detail || nullSpan));
  }.bind(this));
...
MVC._addInterface('date:boot', async function(ev) {
  ...
  let ret = await App.widgets.mdate.calendar(mdt);
...
MVC._addInterface('time:boot', async function(ev) {
  ...
  let ret = await App.widgets.mtime.time(mdt);
  this.$updateModel(path, ret.toJSON());
});
