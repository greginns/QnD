...
class MVC {
  ...
  constructor(element, options) {
    ...
    this._watchedPaths = {};
    this.total = 0;
    this.noBindingList = ['else', 'handle', 'nohandle', 'edit', 'role', 'filter', 'index', 'exclass', 'proxyidx', 'proxypath', 'nonetext', 'nonevalue'];
  ...
  async _init() {
    ...
    this._elementInit(this._section);
  }

  _elementInit(el) {
    for (let sel of el.querySelectorAll('*')) {
//console.log(sel)      
      this._processElement(sel);
    };
  }

  _processElement(el) {
    // for one element find all of it's mvc attributes and run appropriate role and binding
    let pfx = MVC.prefix;
    ...
    if (el.hasAttributes()) {
      ...
      for (let idx=0; idx<attrs.length; idx++) {
        ...
        if (attr.substr(0,pfxLen) == pfx) {   // mvc-something
          attr = attr.substr(pfxLen+1);       // the something

          if (this.noBindingList.indexOf(attr) > -1) continue;  // these aren't bindings, but informational
      ...
      switch (obj.type) {
        case 'CallExpression':
          ...
          if (obj.callee.type == 'Identifier') {
            // first level, ie save()
            funcName = obj.callee.name;
          }
          else {
            // non-first level, ie iofuncs.save(), or funcs.io.save()
            funcName = obj.callee.text;
          }
          ...
          try {
            result = await callee.bind(binder)(fnObj);
          }
          catch (err) {
        ...
        case 'UnaryExpression':
          // argument, operator
          let arg = await parse(obj.argument);
          ...
          if ('object' in obj) {
            ...
            if (mexpr) {
              if (utils.object.isArray(prop)) {
                ...
                result = (prop in mexpr) ? mexpr[prop] : '';
              }
            }
            else {
              result = '';
        ...
        case 'CompoundExpression':
          ...
          for (let p=0; p<obj.leafs.length; p++) {
            let res = await parse(obj.leafs[p]);
  ...
  async _jsGetIdentifiers(ast) {
    ...
    const parse = async function(obj) {
      switch (obj.type) {
        ...
        default:
          console.log('Missing _jsGetIdentifiers', obj.type)
          break;
      }
  ...
  async _jsGetMemberExpressions(ast) {
    ...
    const parse = async function(obj) {
      switch (obj.type) {
        ...
        default:
          console.log('Missing _jsGetMemberExpressions', obj.type)
          break;
      }
...
*/
  async _jsGetIdentifiersAndMemberExpressions(ast) {
    ...
    // *** 2021/10/15  ids and exprs were: [prices, prices[x][y]]  
    // *** So many prices entries that every change to prices caused 42*7 function executions.  
    // *** There were 42*7 prices entries
    let [ids, exprs] = await this._jsGetIdentifiers(ast);
    //let exprs = await this._jsGetMemberExpressions(ast);
    //let tokens = ids.concat(exprs);               // removed  2021/10/15

    let tokens = (exprs.length == 0) ? ids : exprs; // added    2021/10/15
...
//  }
  ...
  $updateModel(path, value) {
    ...
    // only works on single level arrays, ie [1,2,3]
    let ob = path.indexOf('[');
    if (ob > -1) {
      let cb = path.indexOf(']', ob);
      let idxs = (path.substring(ob+1, cb).split(',')).map(function(idx) {
        return idx.trim();
      })

      path = path.substr(0,ob) + '.' + idxs.join('.') + path.substr(cb+1);
    }
  ...
  $addWatched(path, fn, initial) {
    ...
    if (initial) fn(this.$readModel(path), null);
  }
...
// Also removed individual array popping, in favour of one .length at the end.
    ...
    switch(valueType) {
      case 'array':
        ...
        this._doCallBack(key);  // needed when setting a new value to an array.  multiselect example
        break;
