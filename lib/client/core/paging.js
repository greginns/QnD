class Router {
  constructor(root) {
    ...
    this.currentPath = '';
    this.root = (root) ? root + '/' : '';
console.log("Don't forget to add Module.pager = pager.router; to module.js files")    
  }

  add(page) {
    if (Array.isArray(page.path)) {
      for (let pathx of page.path) {
        let path = this.root + Router._stripSlashes(pathx);
  ...
  _linkListener() {
    for (let el of document.getElementsByTagName('A')) {
      if (el.hasAttribute('mvc-nohandle')) {
        // ignore these
        continue;
      }
  ...
  _linkHandler(ev) {
    ...
    var href = ev.target.closest('A').href;
