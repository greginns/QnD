...
class db4ExpressionParser {
  ...
  async evaluate(expr, ctx, opts) {
    this._parserExpr = expr;
    this._dataContext = ctx;
    this._parserOptions = opts || {};
    this._parserPosn = 0;

    let resp = await this._exprParser();
    return resp;
  }
  ...
  async _exprParser() {
    ...
    while (this._parserPosn < this._parserExpr.length) {
      // valid: ' " = { number
      let char = this._parserExpr.charAt(this._parserPosn);
      ...
      if (char == ' ') {
        ...
        resp = await this._parserEq();
      }

      else if (char == "'" || char == '"') {
        resp = this._parserString(char);
      }

      else if (char == this._OPENBRACE) {
        resp = await this._parserObject(char);
      }

      else {
        if (!isNaN(char)) {
          resp = this._parserNumeric(char);
        }
        else {
          if (this._ALPHA.indexOf(char) > -1) {
            resp = this._parserAlphanum(char);
          }
          else {
            resp = '';
          }
  ...
  _parserString(ch) {
    let eq = this._parserExpr.indexOf(ch, this._parserPosn+1);
    if (eq == -1) {
      err = 'No Closing Quote';
      return;
    }

    let token = this._parserExpr.substring(this._parserPosn, eq+1);
    this._parserPosn = eq+1;

    return token;      
  };

  _parserNumeric(token) {
    let char;
    this._parserPosn++;
  ...
  async _parserObject() {
    ...
    let ce = this._parserExpr.indexOf(this._CLOSEBRACE, this._parserPosn);
    let newObj = {}, char, name, value;

    if (ce == -1) {
      let err = 'Missing Close Brace';
      console.log('Missing Close Brace')
      console.log(this._parserExpr, this._parserPosn)
      return;
    }
    ...
    while (true) {
      name = await this._exprParser();
      name = this._parserDequote(name);
      ...
      value = await this._exprParser();
      value = this._parserDequote(value);
      char = this._parserGetNextChar();
  ...
  _parserGetNextChar() {
    ...
    while(true) {
      char = this._parserExpr.charAt(this._parserPosn);
...
db4ExpressionParser.addMethod('process', async function() {
  ...
  let postdata = {};
  let pid = await this._exprParser();
  pid = this._parserDequote(pid);

  let char = this._parserGetNextChar();

  if (char == this._COMMA) {
    postdata = await this._exprParser();
    postdata = this._parserDequote(postdata);
  }
...
db4ExpressionParser.addMethod('tinsert', async function() {
  ...
  let clearIt = false;
  let model = await this._exprParser();
  model = this._parserDequote(model);
  ...
  let table = await this._exprParser();
  table = this._parserDequote(table);

  char = this._parserGetNextChar();

  if (char == this._COMMA) {
    clearIt = await this._exprParser();
    clearIt = this._parserDequote(clearIt);
...
db4ExpressionParser.addMethod('tlookup', async function() {
  ...
  let pks = await this._exprParser();
  let origPks = pks;
  pks = this._parserDequote(pks);
...
db4ExpressionParser.addMethod('tquery', async function() {
  // =tquery(query, model)
  let query = await this._exprParser();
  query = this._parserDequote(query);
...
db4ExpressionParser.addMethod('copyArrayToObject', async function() {
  ...
  let array = await this._exprParser();
  array = this._parserDequote(array);
  ...
  let idx = await this._exprParser();
  idx = this._parserDequote(idx);
  ...
  let obj = await this._exprParser();
  obj = this._parserDequote(obj);
...
db4ExpressionParser.addMethod('form', async function() {
  ...
  fld1 = await this._exprParser();
  fld1 = this._parserDequote(fld1);

  formName = (fld1) ? fld1 : this._parserOptions.form.name || '#' + this._parserOptions.form.id;
  char = this._parserGetNextChar();
  ...
  fldName = await this._exprParser();
  fldName = this._parserDequote(fldName);
  char = this._parserGetNextChar();
