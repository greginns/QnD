...
class Datetime {
  ...
  year(v) {
    v = isNaN(v) ? v = undefined : parseInt(v);

    if (v != undefined) {
      this._datetime.setFullYear(v);
      return this;
    }
  ...
  milliseconds(v) {
    ...
    if (v != undefined) {
      this._datetime.setMilliseconds(v);
      return this;
    }
  ...
  getDaysInMonth() {
    let days = this._daysInMonth[this.month()-1];

    if (this.month() == 2) days += this._leapYearAddition();
  ...
  listOfDays(dt) {
    // [[yr, mo, [days]], [yr, mo, [days]]];
    let tempDt = new Datetime(this);
    let daysBetween = (Math.floor((dt.getValueOf() - tempDt.getValueOf())/86400000)) + 1;
    let prevYy = '', prevMm = '';
    let ret = [], days = [];
    let yy, mm, dd;
    ...
    for (let day=0; day<daysBetween; day++) {
      ...
      if (prevYy != yy || prevMm != mm) {
        ret.push([prevYy, prevMm, days]);
  ...
  isBefore(dt) {
    // this.isBefore(dt)
    return this.getValueOf() < dt.getValueOf();
  }

  isBeforeOrSame(dt) {
    // this.isBeforeOrSame(dt)
    return this.getValueOf() <= dt.getValueOf();
  }  
  ...
  isAfterOrSame(dt) {
    // this.isAfterOrSame(dt)
    return this.getValueOf() >= dt.getValueOf();
  }  

  isSame(dt, asWhat) {
    // this.isSame(dt, 'y|m|d')
  ...
  static timeRound(tm) {
    // hh:mm:ss.mmm --> hh:mm:00.000
    if (!tm) return '00:00:00.000';
