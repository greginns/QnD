...
class WSDataComm {
  ...
  */
  ...
  addModel(model, pgschema) {
    // set list of models we're following.
    // no pgschema default to tenant on server (wouter)
    let modela = model.split('/');  // '/test/testdata'
    pgschema = pgschema || '';
    
    if (modela.length < 2 || modela[1] != this.app) return false;
    
    this.modelURLs.push([model, pgschema]);
    
    if (this.ws) this._subscribeToModelChanges(model, pgschema);
  }

  removeModel(model, pgschema) {
    let idx = this.modelURLs.indexOf(model);
    
    if (this.ws && idx != -1) {
      this.modelURLs.splice(idx);
      this._unsubscribeToModelChanges(model, pgschema);
    }
  }

  _subscribeAllModels() {
    for (let [model, pgschema] of this.modelURLs) {
      this._subscribeToModelChanges(model, pgschema);
    }
  }

  _subscribeToModelChanges(model, pgschema) {
    var msg = JSON.stringify({cat: 'sub', source: 'model', model, 'pgschema': pgschema});

    this.ws.send(msg);
  }
  
  _unsubscribeToModelChanges(model, pgschema) {
    var msg = JSON.stringify({cat: 'unsub', source: 'model', model, 'pgschema': pgschema});
