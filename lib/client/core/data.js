import {io} from '/static/v1/lib/client/core/io.js';
...
class TableStore {
  ...
  */
  constructor({model='', wmodel, safemode=true}={}) {
    this.model = model;
    this.wmodel = wmodel;
  ...
  async getAll() {
    if (this.store.size == 0) {
      let res = await io.get({}, this.model);
  ...
  async getOne(_pk) {
    if (!this.store.has(_pk)) {
      let res = await io.get({}, this.model + '/' + _pk);
      let rows = (res.status == 200) ? res.data : [];
  ...
  async getDefault() {
    let res = await io.get({}, this.model + '/_default');

    return (res.status == 200) ? res.data : {};
  }

  async insert(data) {
    let res = await io.post(data, this.model);
  ...
  async update(_pk, data) {
    let res = await io.put(data, this.model + '/' + encodeURIComponent(_pk));
  ...
  async delete(_pk) {
    let res = await io.delete({}, this.model + '/' + encodeURIComponent(_pk));
  ...
  stop() {
    window.removeEventListener(this.model, this._handleIncoming.bind(this));
  }
