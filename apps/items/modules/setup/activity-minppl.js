...
class Actminppl extends Setup {
  ...
  createModel() {
    ...
    this.model.catname = 'actminppl';
    this.model.itemType = 'Rate'
    this.model.activity = {};
    this.model.actminppl = {};
    this.model.years = [];
    this.model.months = [
  ...
  async ready() {
    return new Promise(async function(resolve) {
      ...
      this.editTable = new Edittable('#actminppl', this, this.saver)
  ...
  async save(ev) {
    ...
    // actminppl is the entry just edited
    let actminppl = this.model.actminppl.toJSON();
    let minppl = this.model.minppls.toJSON();

    // update the one being edited
    minppl[actminppl.dayno] = {minppl: actminppl.minppl};

    minppl = JSON.stringify(minppl);

    let data = {activity: this.activity.code, rateno: this.rateno, year: this.model.year, month: this.model.month, minppl};

    //let spinner = this.startSpinner(ev);

    // new (post) or old (put)?
    let res = (this.model.existingEntry) ? await Module.tableStores.actminp.update([data.activity, data.rateno, data.year, data.month], {minppl: data.minppl}) : await Module.tableStores.actminp.insert(data);
  ...
  async getMinppl() {
    ...
    return await Module.tableStores.actminp.getOne([act, rateno, yy, mm]);
  }

  async canClear(ev) {
    let data = this.model.actminppl.toJSON();
  ...
  async saveRange(obj) {
    ...
    for (let entry of lods) {
      ...
      let res = await Module.tableStores.actminp.getOne([act, rateno, yy, mm]);
      ...
      let data = {activity: act, rateno, year: yy, month: mm, minppl};
  
      // new (post) or old (put)?
      res = (existingEntry) ? await Module.tableStores.actminp.update([data.activity, data.rateno, data.year, data.month], {minppl: data.minppl}) : await Module.tableStores.actminp.insert(data);
...
let el1 = document.getElementById('items-minppl-activity');   // page html
let setup1 = new Actminppl('items-minppl-activity-section');
let section1 = new Section({mvc: setup1});
let page1 = new Page({el: el1, path: ['/activity/:code/rate/:rateno/minppl'], title: 'Activity Minimum People', sections: [section1]});
